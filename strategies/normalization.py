...
class SoftClipStrategy(NormalizationStrategy):
    """Use tanh to gently squash spikes towards +/- 100 without hard clipping."""
    def process(self, series, window=5, limit=100, scale=35, **kwargs):
        roc = (series.pct_change() * 100).replace([np.inf, -np.inf], 0).fillna(0)
        # Smoothly squashes large values. Linear near 0, approaches 'limit' for large values.
